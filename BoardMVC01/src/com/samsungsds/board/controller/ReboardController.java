...
public class ReboardController extends HttpServlet {
	...
	private void execute(HttpServletRequest request,
		...
		if (act.equals("mvnew")) {
			...
			int seq = reboardService.getNextSeq();
			...
			int cnt = reboardService.writeNewArticle(reboardDto);
			...
			flag = false;
		} else if (act.equals("mvreply")) {
			int seq = StringCheck.stringToZero(request.getParameter("seq"));

			ReboardDto reboardDto = reboardService.getArticle(seq);

			request.setAttribute("reboardArticle", reboardDto);
			path = "/board/reply.jsp" + queryString;
			flag = false;
		} else if (act.equals("replywrite")) { 
			HttpSession session = request.getSession();
			MemberDto memberDto = (MemberDto) session.getAttribute("userInfo");
			...
			int cnt = reboardService.writeReplyArticle(reboardDto);
